	...
	public class TextNavigator
	{
		...
		internal virtual void BeginEditMode()
		{
			if(!EditMode)
			{
				...
				basicTextSelectionHandler.Setup(InputField.TextRenderer.transform, this);
				...
				if(InputField.CanUseTouchSelectionCursors || InputField.CanUseActionBar)
				{
					...
					touchTextSelectionHandler.Setup(InputField.TextRenderer.transform, this);
		...
		internal void KeepActionBarWithinBounds()
		{
			...
			float normalizedLeftX;
			float normalizedTopY;
			float normalizedRightX;
