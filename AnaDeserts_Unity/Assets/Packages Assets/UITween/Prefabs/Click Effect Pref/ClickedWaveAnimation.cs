...
public class ClickedWaveAnimation : MonoBehaviour
{
    ...
    void CreateWave(Transform Parent)
    {
        ...
        if (wave)
        {
            ...
            MaskableGraphic maskGraph = wave.GetComponent<MaskableGraphic>();

            maskGraph.color = Parent.GetComponent<MaskableGraphic>().color - new Color(.1f, .1f, .1f);
            //Color desired = maskGraph.color;
            //desired.a = 0.4f;
            //maskGraph.color = desired;
    ...
    public GameObject UiHitted()
    {
        ...
        //}
        if (hits.Count > 0)
            if (hits[0].gameObject.GetComponent<Button>() && hits[0].gameObject.GetComponent<Mask>())
